...
static void LORA_HasJoined(void)
{
  ...
  PRINTF("JOINED\n\r");
		...
		result=ModbusMaster_readHoldingRegisters(0x01,0x1,2);
				if (result == 0x00)
		{
...
static void Send(void *context)
{
  ...
  AppData.Buff[i++] = Input_Result[0];
  AppData.Buff[i++] = Input_Result[1];
...
static void MX_TIM6_Init(void)
{
  ...
  /* USER CODE BEGIN TIM6_Init 2 */
	__HAL_TIM_URS_ENABLE(&htim6);    //ENABLE COUNTER OVER FLOW INTERRUPT
  __HAL_TIM_CLEAR_IT(&htim6, TIM_IT_UPDATE)	;	//CLEAR INTERRUPT BIT
...
static void MX_TIM7_Init(void)
{
  ...
  /* USER CODE BEGIN TIM7_Init 2 */
	__HAL_TIM_URS_ENABLE(&htim7);    //ENABLE COUNTER OVER FLOW INTERRUPT
  __HAL_TIM_CLEAR_IT(&htim7, TIM_IT_UPDATE)	;	//CLEAR INTERRUPT BIT
...
static void MX_USART2_UART_Init(void)
{
  ...
  /* USER CODE BEGIN USART2_Init 2 */
	__HAL_UART_ENABLE_IT(&huart2,UART_IT_RXNE);   //added
